...
const createOpenAIClient = (): AzureOpenAI => {
...
const getMCPServerTools = async (mcpId: string): Promise<Record<string, any> | null> => {
...
const validateMCPServerTool = (mcpId: string, toolName: string): boolean => {
...
export class MCPIdentificationAgent {
  ...
  public get registry(): MCPRegistry {
    return this.mcpRegistry;
  }

  public async identifyAndPrepareServer(command: string): Promise<MCPServerIdentification> {
    // Use Azure OpenAI to identify MCP
    const allMCPs = this.mcpRegistry.getAllServerConfigs();
    const mcpDescriptions = allMCPs.map(mcp => 
      `${mcp.id}: ${mcp.name} - ${mcp.description || "No description"}`
    ).join('\n');
...
DO NOT WRITE ANYTHING ELSE. JUST RESPOND WITH THE JSON OBJECT.`;
    ...
    try {
      ...
      return this.validateServer(
        result.mcpId === 'unknown' ? undefined : result.mcpId,
        result.mcpName === null ? undefined : result.mcpName
      );
    } catch (error) {
  ...
  public async validateServer(
    mcpId: string | undefined, 
    mcpName: string | undefined
  ): Promise<MCPServerIdentification> {
  ...
  public async process(request: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        error: `Error identifying MCP server: ${(error as Error).message}`
      };
    }
  }
}
...
export class CommandIntentAgent {
  ...
  public async identifyTool(command: string, mcpId: string): Promise<ToolIdentification> {
...
Identify the most appropriate tool. Respond in JSON:
{
  ...
  public async process(request: any): Promise<any> {
    try {
      ...
      return {
        success: false,
        error: `Error identifying tool: ${(error as Error).message}`
      };
    }
  }
}
  ...
  "reasoning": "Brief explanation"
    ...
    try {
      ...
      const missingRequired = (tool.inputSchema?.required || [])
        .filter((param: string) => !(param in (result.parameters || {})));
